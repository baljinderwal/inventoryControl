...
const StockAdjustmentForm = ({
  ...
  return ( <
    ...
    Select labelId = "adjustment-type-label"
    id = "adjustment-type"
    value = {
      adjustmentType
    }
    onChange = {
      (e) => setAdjustmentType(e.target.value)
    }
    ...
    onChange = {
      (e) => setQuantity(Math.max(1, parseInt(e.target.value, 10)))
    }
    InputProps = {
      {
        inputProps: {
...

    {
      adjustmentType === 'in' && ( <
        ...
        onChange = {
          (e) => setBatchNumber(e.target.value)
        }
        required /
        >
        <
    ...
    {
      mutation.isPending ? < CircularProgress size = {
        24
      }
      /> : 'Submit Adjustment'} <
      /Button> <
